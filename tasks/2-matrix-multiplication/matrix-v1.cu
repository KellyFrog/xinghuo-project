...
__global__ void KMatrixMul(const float* a, const float* b, std::size_t n, std::size_t m, std::size_t k, float* c) {
...
void MatrixMul(const float* a, const float* b, std::size_t n, std::size_t m, std::size_t k, float* c) {
	KMatrixMul<<<dim3(n / BSIZE + 1, k / BSIZE + 1), dim3(BSIZE, BSIZE)>>>(a, b, n, m, k, c);
}
